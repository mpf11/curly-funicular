...
internal static class WheelGeometry
{
    ...
    internal static double SliceBoundaryAngleDeg(int i) =>
        -90.0 - SliceSpanDeg / 2.0 + i * SliceSpanDeg;
    ...
    /// </summary>
    internal static int PointToSlot(
        double dx,
        double dy,
        double innerRadius,
        double outerRadius,
    ...
    )
    {
        ...
        if (dist < innerRadius)
            return -1;
        if (requireInBounds && dist > outerRadius)
            return -1;
